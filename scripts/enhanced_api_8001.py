"""
Enhanced A/B Testing API on port 8001
"""
...
class PredictionResponse(BaseModel):
    ...
    feature_interpretation: Optional[dict] = None
...
def interpret_features(features):
    ...
    return {
        "loan_amount": f"",
        "annual_income": f"", 
        "credit_score": f"{credit_score:.0f}",
        "debt_to_income_ratio": f"{debt_ratio:.1%}",
        "employment_years": f"{employment:.1f} years",
        "loan_to_income_ratio": f"{(loan_amount/income):.1%}" if income > 0 else "N/A"
    }
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        feature_interp = interpret_features(request.features)
